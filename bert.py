...
class BertSelfAttention(nn.Module):
    ...
    def attention(self, key, query, value, attention_mask):
        ...
        # each attention is calculated following eq (1)of "All you need is Love"
        ...
        attention_probs = F.softmax(attention_scores, dim = -1) # F : functional
...
class BertLayer(nn.Module):
    ...
    def add_norm(self, input, output, dense_layer, dropout, ln_layer):
        ...
        """
        # TODO
        hidden_states = dropout(dense_layer(output))
            
        return ln_layer(input + hidden_states)    
    ...
    """
        ...
        # BertSelfAttention / self.self_attention = BertSelfAttention(config)
        ...
        activated_output = self.interm_af(interm_output)
        #ff_output = self.out_dense(ff_output)
        
        # another add-norm layer
        ff_output = self.add_norm(attention_output, activated_output, self.out_dense, self.out_dropout, self.out_layer_norm)
...
"""
Shape after add-norm :  torch.Size([2, 8, 768])
Shape interm :  torch.Size([2, 8, 3072])
Shape activated :  torch.Size([2, 8, 3072])
Shape out_dense :  torch.Size([2, 8, 768])
dense layer,: Linear(in_features=3072, out_features=768, bias=True)  / output: torch.Size([2, 8, 768])
"""
...
class BertModel(BertPreTrainedModel):        
    ...
    def embed(self, input_ids):
        ...
        seq_length = input_shape[1] # input_ids.size(1)

        # TODO : get word embedding from self.word_embedding
